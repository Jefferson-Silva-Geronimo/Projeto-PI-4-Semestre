...
import { CreatePreferenceDTO } from "./payment.types";

export class PaymentService {
  async createPreference(data: CreatePreferenceDTO, userId: string) {
    ...
    const items = data.items.map((item) => {
      ...
      if (!product || item.quantity > product.stock) {
        throw new Error(
          `Estoque insuficiente para ${product?.name ?? "o produto"}.`,
        );
      }
